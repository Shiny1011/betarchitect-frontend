      ...
      <div className='bg-dark-indigo mb-6 flex w-full space-x-6 rounded-2xl p-3 md:w-fit'>
        {tabs.map((tab) => (
          <button
            ...
            className={`h-[35px] w-full rounded-xl px-4 py-2 text-base font-medium transition-colors duration-200 md:h-[47px] md:w-[133px] md:text-sm ${
          ...
          <table className='w-full'>
            <thead className='bg-dark-indigo'>
              <tr>
                ...
                <th className='text-lavander/50 px-4 py-3 text-left text-sm font-medium'>Player</th>
                <th className='text-lavander/50 px-4 py-3 text-left text-sm font-medium'>Time</th>
                <th className='text-lavander/50 px-4 py-3 text-left text-sm font-medium'>Bet</th>
                <th className='text-lavander/50 px-4 py-3 text-left text-sm font-medium'>Multiplier</th>
            ...
            <tbody className='divide-y divide-gray-700'>
              {mockBetData.map((bet, index) => (
                <tr
                  key={bet.id}
                  className={`${index % 2 === 0 ? 'bg-dark-primary' : 'bg-[#22355A]'} transition-colors duration-200`}
                >
                  {/* Game Column */}
                  <td className='px-4 py-3'>
                    <div className='flex items-center space-x-2'>
                      <span className='text-lg'>{bet.game.icon}</span>
                      <span className='text-lavander text-base font-semibold'>{bet.game.name}</span>
                    </div>
                  </td>
                  ...
                  <td className='px-4 py-3 text-right'>
                    <span className='text-base font-semibold'>{bet.payout.toFixed(2)}$</span>
                  </td>
                </tr>
              ))}
            </tbody>
